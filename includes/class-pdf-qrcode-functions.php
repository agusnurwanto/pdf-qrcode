...
class QRCODE_Functions
{
    ...
    function allow_access_private_post() {
		if (
			...
			$key_db = md5(get_option( QRCODE_APIKEY ));
    ...
    public static function uploadFile(
		...
		try {
			if (!empty($api_key) && $api_key == get_option( QRCODE_APIKEY )) {
				if (!empty($file)) {
	...
	public function check_force_password_change() {
		...
		$force_change = get_user_meta($user_id, 'QRCODE_force_password_change', true);
	...
	public function ajax_change_password() {
		...
		if (!empty($_POST)) {
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option(QRCODE_APIKEY)) {
				...
				if (empty($new_password)) {
					...
					delete_user_meta($user_id, 'QRCODE_force_password_change');
	...
	public function shortcode_ubah_password($atts) {
		...
		<div class="container" style="max-width: 500px; margin: 50px auto;">
			<input type="hidden" value="<?php echo get_option(QRCODE_APIKEY); ?>" id="api_key" />
